...
// Event / celebration themed images for an event management company
...
export default function DomeGallery({
  ...
  const startInertia = useCallback((vx, vy) => {
    ...
    const stopThresh   = 0.015 - 0.01 * d;
    const maxFrames    = Math.round(90 + 270 * d);

    const step = () => {
      vX *= frictionMul; vY *= frictionMul;
      if (Math.abs(vX) < stopThresh && Math.abs(vY) < stopThresh) { inertiaRAF.current = null; return; }
